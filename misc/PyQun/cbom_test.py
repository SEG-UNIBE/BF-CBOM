            ...
            if elements and len(elements) > 1:
                ...
                for e in elements:
                    ...
                    try:
                        ...
                        group_indices.append((file_id, comp_id))
                    except:
                        print(f"Error during extraction of component: {comp_id} from file: {file_id}")
                grouped_elements.append(group)
                grouped_elements_indices.append(group_indices)

    return grouped_elements, grouped_elements_indices

try:
    files = ["CodePhiliaX_Chat2DB_cbomkit.json", "CodePhiliaX_Chat2DB_cdxgen.json"]

    json_files = load_jsons_from_files_as_strings(files)

    grouped_matches, _ = match_from_json_list(json_files)

    for i, g in enumerate(grouped_matches):
        print(f"Group: {i}")

        for e in g:
            print(f"    - file: {str(e)[:200]}")
